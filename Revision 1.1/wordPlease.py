...
# If this cnt is not max contour
	...
	if((len(cnt) != len(contour_max)) and (cv2.contourArea(cnt) > 250)):
...
WORD = cv2.bitwise_and(rotated,rotatedMask)
...
# cv2.imshow("rotated", WORD)
a = box[1]
c = box[3]
# print "a is ", a , "c is ", c
if((c[1] < a[1]) and (c[0] < a[0])):

	roi = WORD[c[1]:a[1],c[0]:a[0]]
	print roi
	cv2.imshow('ROI',roi)
	cv2.imwrite('word.jpg',roi)
elif ((c[1] < a[1]) and (c[0] > a[0])) :
	roi = WORD[c[1]:a[1],a[0]:c[0]]
	print roi
	cv2.imshow('ROI',roi)
	cv2.imwrite('word.jpg',roi)
	
elif ((c[1] > a[1]) and (c[0] < a[0])) :
	roi = WORD[a[1]:c[1],c[0]:a[0]]
	print roi
	cv2.imshow('ROI',roi)
	cv2.imwrite('word.jpg',roi)
elif ((c[1] > a[1]) and (c[0] > a[0])) :
	roi = WORD[a[1]:c[1],a[0]:c[0]]
	print roi
	cv2.imshow('ROI',roi)
	cv2.imwrite('word.jpg',roi)
