...
_,binary = cv2.threshold(cv2.cvtColor(frame, cv2.cv.CV_BGR2GRAY), 150,255,cv2.THRESH_BINARY)
# binary =  cv2.bitwise_or(cv2.bitwise_not(deathMask),binary)

cv2.imshow('Binary Morphed with MASK',binary)
...
#cv2.cvtColor(rotated_bro, cv2.cv.CV_BGR2GRAY)
_,binaryWord = cv2.threshold(cv2.cvtColor(rotatedMask, cv2.cv.CV_BGR2GRAY), 5,255,cv2.THRESH_BINARY)
...
b = box[2]
c = box[3]
print "a is ", a , "c is ", c, "b is ", b
print (c[1] < a[1]) and (c[0] < a[0]), (c[1] < a[1]) and (c[0] > a[0]), (c[1] > a[1]) and (c[0] < a[0]), (c[1] > a[1]) and (c[0] > a[0])
print a[0],a[1],c[0],c[1]

if((c[1] <= a[1]) and (c[0] <= a[0])):

	roi = rotated_bro[c[1]:a[1],c[0]:a[0]]
	print roi
	cv2.imshow('ROI',roi)
	cv2.imwrite('word.jpg',roi)
elif ((c[1] <= a[1]) and (c[0] >= a[0])) :
	print "Inside her"
	roi = rotated_bro[c[1]:a[1],a[0]:c[0]]
	...
	roi = rotated_bro[a[1]:c[1],c[0]:a[0]]
	print roi
	cv2.imshow('ROI',roi)
	cv2.imwrite('word.jpg',roi)
elif ((c[1] >= a[1]) and (c[0] >= a[0])) :
